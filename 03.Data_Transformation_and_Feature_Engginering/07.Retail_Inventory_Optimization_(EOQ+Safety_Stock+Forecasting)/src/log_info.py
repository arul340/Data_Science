...
def setup_logger(config: dict = None) -> logging.Logger :
    ...
    log_file_config = Path(config["paths"]["logs"]["log_file"])
    log_file_config.parent.mkdir(parents=True, exist_ok=True)

    # -> Create Logger
    logger = logging.getLogger("Inventory Optimization Pipeline")
    runtime_cfg = (config.get("runtime") or config.get("run_time") or {})
    ...
    # -> Formatter 
    log_format = runtime_cfg.get("log_format", "[%(asctime)s] [%(levelname)s] [%(name)s] %(message)s")
    date_format = runtime_cfg.get("log_datefmt", "%Y-%m-%d %H:%M:%S")

    formatter = logging.Formatter(fmt=log_format, datefmt=date_format)

    # -> Console Handler 
    console_handler = logging.FileHandler(log_file_config)
